  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec mark_loc loc =
        if (List.mem loc !reachable_locs) then ()
        else (
          reachable_locs := [loc] @ (!reachable_locs);
          let (x, y) = loc in
          let tf ((x', y'), z') = (x=x') in
          let loc' = List.find tf m in
          match loc' with
          | (z, L loc'') -> mark_loc loc''
          | (z, R loc'') -> 
            let tfl (w, l) = mark_loc l in
            recur (tfl, loc'')
          | (loc'', z) -> mark_loc loc''
        )
      in
    
      let rec mark_env (x, eval) = 
        match eval with
        | Loc loc -> mark_loc loc 
        | Proc (y, comm, env') -> 
          let tfe (env'') = mark_env env'' in
          recur (tfe, env')
      in
      let _ = let tfe (env'') = mark_env env'' in recur (tfe, e) in
      ...
      let rec mark_stack stack = 
        ...
        | P (x, y, env) -> 
          let tfe (env'') = mark_env env'' in
          recur (tfe, env)
        | M eval -> mark_env eval
      in
      let _ = let tfs (stack) = mark_stack stack in recur (tfs, s) in

      
      let rec mark_cont (comm, env') = 
        let tfe (env'') = mark_env env'' in
        recur (tfe, env')
      in
      let _ = let tfc (x, cont) = mark_cont (x, cont) in recur (tfc, k) in
