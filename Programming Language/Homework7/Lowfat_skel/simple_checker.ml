...
let rec malgorithm (env, exp, typs) = 
  ...
  | M.FN (v, e) ->
    ...
    let f = TFun (v1, v2) in
    let subs1 = unify (env, f, typs) in
    let env' = [v, (subs1 v1)] @ env_subs (env, subs1) in
    let subs2 = malgorithm (env', e, (subs1 v2)) in
    (fun t' -> subs1 (subs2 t'))
  | M.APP (e1, e2) -> 
    let v = TVar (new_var()) in
    let subs1 = malgorithm (env, e1, TFun (v, typs)) in
    let v' = (subs1 v) in
    let env' = env_subs (env, subs1) in
    let subs2 = malgorithm (env', e2, v') in
    (fun t' -> subs1 (subs2 t'))
  ...
  | M.MALLOC e -> 
    let v = TVar (new_var()) in
    let subs1 = unify (env, TLoc v, typs) in
    let env' = env_subs (env, subs1) in 
    let subs2 = malgorithm (env', e, (subs1 v)) in
    (fun t' -> subs1 (subs2 t'))
  | M.ASSIGN (e1, e2) ->
    let subs1 = malgorithm (env, e1, TLoc typs) in
    let env' = env_subs (env, subs1) in 
    let subs2 = malgorithm (env', e2, (subs1 typs)) in
    (fun t' -> subs1 (subs2 t'))
  | M.BANG e -> malgorithm (env, e, (TLoc typs))
  | M.SEQ (e1, e2) ->
    let v = TVar (new_var()) in
    let subs1 = unify (env, v, typs) in
    let env' = env_subs (env, subs1) in
    let subs2 = malgorithm (env', e2, (subs1 v)) in
    (fun t' -> subs1 (subs2 t'))
  | M.PAIR (e1, e2) ->
    let v1 = TVar (new_var()) in
    let v2 = TVar (new_var()) in
