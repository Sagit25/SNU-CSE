...
struct
  ...
  let op2fn =
    ...
    | EQ -> (fun (v1,v2) -> 
      ...
      | _ -> raise (TypeError "Wrong Compare!")
      )
    ...
    | LET (e, e2) -> 
      (
        ...
        | VAL (x, e1) ->
          ...
          eval env m' e2
        | REC (f, x, e1) ->
          let rf = RecFun (f, x, e1) in
          let env = env @+ (f, Closure (rf, env)) in
          eval env mem e2
      )
    | MALLOC e -> 
      let (v, m') = eval env mem e in
      let (loc, m'') = malloc m' in
      (Loc loc, store m'' (loc, v))
    | ASSIGN (e1, e2) ->
      let (r, m') = eval env mem e1 in
      let (v, m'') = eval env m' e2 in
      (v, store m'' (getLoc r, v))
    | BANG e -> 
      let (r, m') = eval env mem e in
      (load m' (getLoc r), m')
    | SEQ (e1, e2) ->
      let (x, m') = eval env mem e1 in
      eval env m' e2
